...
const (
	ffj_t_MtBroadCastMessagebase = iota
	ffj_t_MtBroadCastMessageno_such_key

	ffj_t_MtBroadCastMessage_Clients

	ffj_t_MtBroadCastMessage_Payload
)

var ffj_key_MtBroadCastMessage_Clients = []byte("topics")

var ffj_key_MtBroadCastMessage_Payload = []byte("payload")
...
func (uj *MtBroadCastMessage) UnmarshalJSONFFLexer(fs *fflib.FFLexer, state fflib.FFParseState) error {
	var err error = nil
	currentKey := ffj_t_MtBroadCastMessagebase
	_ = currentKey
	tok := fflib.FFTok_init
	wantedTok := fflib.FFTok_init
...
mainparse:
	for {
		...
		case fflib.FFParse_want_key:
			...
			if len(kn) <= 0 {
				// "" case. hrm.
				currentKey = ffj_t_MtBroadCastMessageno_such_key
				state = fflib.FFParse_want_colon
				goto mainparse
			} else {
				switch kn[0] {
				...
				case 'p':
					...
					if bytes.Equal(ffj_key_MtBroadCastMessage_Payload, kn) {
						currentKey = ffj_t_MtBroadCastMessage_Payload
						state = fflib.FFParse_want_colon
						goto mainparse
					}
				...
				case 't':
					...
					if bytes.Equal(ffj_key_MtBroadCastMessage_Clients, kn) {
						currentKey = ffj_t_MtBroadCastMessage_Clients
						state = fflib.FFParse_want_colon
						goto mainparse
					}
				...
				if fflib.SimpleLetterEqualFold(ffj_key_MtBroadCastMessage_Payload, kn) {
					currentKey = ffj_t_MtBroadCastMessage_Payload
					state = fflib.FFParse_want_colon
					goto mainparse
				}

				if fflib.EqualFoldRight(ffj_key_MtBroadCastMessage_Clients, kn) {
					currentKey = ffj_t_MtBroadCastMessage_Clients
					state = fflib.FFParse_want_colon
					goto mainparse
				}
		...
		case fflib.FFParse_want_value:
			...
			if tok == fflib.FFTok_left_brace || tok == fflib.FFTok_left_bracket || tok == fflib.FFTok_integer || tok == fflib.FFTok_double || tok == fflib.FFTok_string || tok == fflib.FFTok_bool || tok == fflib.FFTok_null {
				...
				case ffj_t_MtBroadCastMessage_Clients:
					goto handle_Clients

				case ffj_t_MtBroadCastMessage_Payload:
					goto handle_Payload

				case ffj_t_MtBroadCastMessageno_such_key:
					err = fs.SkipField(tok)
					if err != nil {
						return fs.WrapErr(err)
					}
